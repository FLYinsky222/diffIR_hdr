...
"""
HDR任务专用训练流程脚本
简化版本，移除了渐进式训练，专注于LDR到HDR的图像恢复任务
"""
import datetime
import logging
import math
...
def train_pipeline_hdr(root_path):
    """HDR任务专用训练流程主函数"""
    # 解析选项，设置分布式设置，设置随机种子
    ...
    # 否则日志记录器将无法正确初始化
    log_file = osp.join(opt['path']['log'], f"train_hdr_{opt['name']}_{get_time_str()}.log")
    ...
    logger.info(f"HDR训练配置:")
    logger.info(f"  - 批次大小: {batch_size}")
    logger.info(f"  - 图像尺寸: {gt_size}x{gt_size}")
    logger.info(f"  - 总迭代数: {total_iters}")

    # 开始训练
    logger.info(f'开始HDR训练，从轮次: {start_epoch}, 迭代: {current_iter}')
    ...
    logger.info(f'HDR训练结束。消耗时间: {consumed_time}')
    logger.info('保存最新模型。')
